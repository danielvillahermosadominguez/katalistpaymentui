...
jest.mock("../src/js/location/location.js", () => ({
  ...
  localizeHTMLTagForAllOptionsInSelector:
    MOCK_LOCALIZE_HTML_TAG_FOR_ALL_OPTIONS_IN_SELECTOR,
  t: MOCK_T
}));
...
describe("The form with the payment data (Step 2) should", () => {
  ...
  beforeEach(async () => {
    ...
    MOCK_LOCALIZE_HTML_TAG_FOR_ALL_OPTIONS_IN_SELECTOR.mockImplementation(
      (selectorId, optionIdPrefix, sortByText) => {
  ...
  renderOnlyHtml = async () => {
    ...
    const html = fs.readFileSync(
      path.resolve(__dirname, "../src/views/payment/payment.html"),
      "utf8"
    );
  ...
  describe("show several fields and buttons in the form", () => {
    ...
    it("show the confirm purchase button", () => {
      const buttonConfirmPurchase = document.getElementById(
  ...
  describe("allow to choose a date", () => {
    beforeEach(async () => {
      setHref("?lang=en&course=10");
      await loadJs();
    });

    it("show by default MM and YY ", async () => {
      const month = document.getElementById("select-month");
      const year = document.getElementById("select-year");
      var selectedMonthOption = month.options[month.selectedIndex];
      var selectedMonth = selectedMonthOption.innerHTML;
      var selectedYearOption = year.options[year.selectedIndex];
      var selectedYear = selectedYearOption.innerHTML;

      expect(month.value).toBe("[option_month_mm]");
      expect(selectedMonth).toBe("[option_month_mm]");
      expect(year.value).toBe("YY");
      expect(selectedYear).toBe("YY");
    });
    ...
    it.each([
      ...
      "select and show the month '%s' with the value '%s'",
      async (_, code) => {
        const month = document.getElementById("select-month");
        const option = document.getElementById("option_month_" + code);

        option.selected = true;
        month.dispatchEvent(new Event("change"));

        await new Promise(process.nextTick);
        var selectedOption = month.options[month.selectedIndex];
        var selectedMonth = selectedOption.innerHTML;
        expect(month.value).toBe(code);
        expect(selectedMonth).toBe("[option_month_" + code + "]");
      }
    );
  });
  ...
  it.each([
    ...
    ["2031", "31"],
    ["2032", "32"]
  ])("select and show the a year %s", async (year, code) => {
    const yearSelector = document.getElementById("select-year");
    const option = document.getElementById("option_year_" + code);

    option.selected = true;
    yearSelector.dispatchEvent(new Event("change"));

    await new Promise(process.nextTick);
    var selectedOption = yearSelector.options[yearSelector.selectedIndex];
    var selectedYear = selectedOption.innerHTML;
    expect(yearSelector.value).toBe(code);
  ...
  describe("prepare an integration with paycomets", () => {
    ...
    it("the number of credit/debit card must be present for the integration", () => {
      ...
      expect(cardNumber.getAttribute("paycomet-placeholder")).toBe(
  ...
  describe("subscribe and pay", () => {
    ...
    it("show an error when not connect with service", async () => {
      ...
      expect(dom.locationReplace.mock.calls).toHaveLength(1);
      expect(dom.locationReplace.mock.calls[0][0]).toBe(
        ERROR_GENERIC + "?lang=en&message=[there_is_not_connection]"
      );
    });
    ...
    it("show an error when the course doesn't exist", async () => {
      ...
      expect(dom.locationReplace.mock.calls).toHaveLength(1);
      expect(dom.locationReplace.mock.calls[0][0]).toBe(
        ERROR_GENERIC + "?lang=en&message=[backend_error_code_1]"
      );
    });
    ...
    it("show an error when the user has already a subscription to this course", async () => {
      ...
      expect(dom.locationReplace.mock.calls).toHaveLength(1);
      expect(dom.locationReplace.mock.calls[0][0]).toBe(
        ERROR_GENERIC + "?lang=en&message=[backend_error_code_2]"
      );
    });
    ...
    it("show an error when there is a general error in the subscription", async () => {
      ...
      expect(dom.locationReplace.mock.calls).toHaveLength(1);
      expect(dom.locationReplace.mock.calls[0][0]).toBe(
        ERROR_GENERIC + "?lang=en&message=[backend_error_code_3]"
      );
    });
    ...
    it("show an error when the price for this course is not found", async () => {
      ...
      expect(dom.locationReplace.mock.calls).toHaveLength(1);
      expect(dom.locationReplace.mock.calls[0][0]).toBe(
        ERROR_GENERIC + "?lang=en&message=[backend_error_code_4]"
      );
    });
    ...
    it("show an error when the url challenge empty", async () => {
      ...
      expect(dom.locationReplace.mock.calls).toHaveLength(1);
      expect(dom.locationReplace.mock.calls[0][0]).toBe(
        ERROR_GENERIC + "?lang=en&message=[problem_with_payment]"
      );
    });
